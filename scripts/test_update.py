...
def main():
    ...
    client.table("report_jobs").insert(
        {"id": job_id, "user_id": "test_update_user", "status": "pending"}
    ).execute()

    # Try update
    logger.info("Attempting update...")
    try:
        client.table("report_jobs").update({"status": "running"}).eq("id", job_id).execute()
        # Verify
        ...
        if data and data[0]["status"] == "running":
            ...
            logger.error(
                f"Update FAILED. Status: {data[0]['status'] if data else 'None'}"
            )
