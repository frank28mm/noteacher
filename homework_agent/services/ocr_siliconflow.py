...
class SiliconFlowDeepSeekOCRClient:
    ...
    def _build_client(self) -> OpenAI:
        return OpenAI(base_url=self.base_url, api_key=self.api_key, timeout=float(self.timeout_seconds))
    ...
    def analyze(self, *, image_url: str) -> SiliconFlowOCRResult:
        if not self.is_configured():
            raise RuntimeError("SILICON_API_KEY/SILICON_BASE_URL/SILICON_OCR_MODEL not configured")
        if not image_url:
        ...
        if obj is None:
            ...
            data_uri = _as_data_uri(image_url, timeout_seconds=float(self.timeout_seconds))
            if data_uri:
                log_event(
                    logger,
            ...
            if isinstance(raw_blocks, list):
                for b in raw_blocks:
                    ...
                    if isinstance(bbox, (list, tuple)) and len(bbox) == 4:
                        ...
                        vals = [_to_float(bbox.get(k)) for k in ("xmin", "ymin", "xmax", "ymax")]
                        if all(v is not None for v in vals):
                            coords = [float(v) for v in vals]  # type: ignore[list-item]
