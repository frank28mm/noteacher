...
Usage:
    ...
    python scripts/collect_metrics.py --image-dir /path/to/images --mode local

    # URL mode (uploads to Supabase first, then uses URL):
    python scripts/collect_metrics.py --image-dir /path/to/images --mode url

    # Single image:
    python scripts/collect_metrics.py --image /path/to/image.jpg --mode local
...
def find_images(directory: str, extensions: tuple = (".jpg", ".jpeg", ".png", ".webp")) -> List[Path]:
...
def upload_to_supabase(path: Path, max_side: int = 1024, quality: int = 80) -> Optional[str]:
...
async def main():
    ...
    parser.add_argument("--mode", choices=["local", "url"], default="local", help="Image delivery mode")
    ...
    parser.add_argument("--max-side", type=int, default=1024, help="Max image dimension")
    ...
    parser.add_argument("--output", type=str, default="metrics.json", help="Output file")
    ...
    for i, img_path in enumerate(images[:args.limit], 1):
        ...
        m = await run_single(img_path, args.mode, subject, args.provider, args.max_side, args.quality)
        metrics.append(m)
