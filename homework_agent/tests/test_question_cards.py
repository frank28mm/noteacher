...
def test_infer_answer_state_prefers_answer_status():
    ...
    assert infer_answer_state(student_answer=None, answer_status="已作答") == "has_answer"
...
def test_infer_answer_state_student_answer_variants():
    ...
    assert (
        infer_answer_state(student_answer="（未提取到，题干/作答请参考 vision_raw_text）", answer_status=None)
        == "unknown"
    )
...
def test_merge_and_sort_question_cards():
    merged = merge_question_cards(
        {},
        [
            {"item_id": "p2:q:1", "question_number": "1", "page_index": 1, "card_state": "placeholder"},
            {"item_id": "p1:q:2", "question_number": "2", "page_index": 0, "card_state": "placeholder"},
        ],
    )
    merged = merge_question_cards(
        merged,
