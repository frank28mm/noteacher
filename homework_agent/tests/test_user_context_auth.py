...
def test_require_user_id_prefers_bearer_token(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setattr(
        uc, "_verify_supabase_jwt", lambda token: "user_123" if token == "t" else None
    )
    monkeypatch.setattr(
        uc, "get_settings", lambda: types.SimpleNamespace(auth_required=False)
    )
...
def test_require_user_id_auth_required_raises_401_when_missing(
    monkeypatch: pytest.MonkeyPatch,
):
    monkeypatch.setattr(
        uc, "get_settings", lambda: types.SimpleNamespace(auth_required=True)
    )
    with pytest.raises(HTTPException) as ei:
...
def test_require_user_id_falls_back_to_dev_when_not_required(
    monkeypatch: pytest.MonkeyPatch,
):
    monkeypatch.setattr(
        uc, "get_settings", lambda: types.SimpleNamespace(auth_required=False)
