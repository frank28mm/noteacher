...
async def test_real_image_autonomous_agent():
    ...
    print(f"\n{'='*60}")
    print(f"Testing Autonomous Agent with real image")
    ...
    print(f"Incorrect: {sum(1 for r in result.results if r.get('verdict') == 'incorrect')}")
    print(f"Uncertain: {sum(1 for r in result.results if r.get('verdict') == 'uncertain')}")
