...
7. **产品化数据路径（Submission）**：前端只上传原始文件给后端；后端按 `user_id` 存储原图并返回 `upload_id`（一次上传=一次 Submission），后续 `/grade`/`/chat`/qindex/relook 都围绕这份“权威原图”展开。
...
9. **Chat “看图”保底与禁止嘴硬**：
   - 用户要求“看图/图形/表格”等时：必须触发 qindex 并等待切片；切片 ready 后 relook；若仍失败则明确回复“看不到图”，禁止臆测式解释；
