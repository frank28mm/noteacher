...
- `homework_agent/API_CONTRACT.md`：接口契约（`/grade` `/chat` `/jobs` 请求/响应字段、SSE 事件、错误码、幂等、超时/重试）。
...
- `docs/cto_90_day_plan.md`：路线图（唯一一份“交付节奏”文档）。
...
- 会话/记忆边界：
  - chat 会话/对话历史按 `session_id` 24 小时生命周期，超期不保证恢复对话；
...
- SSE：心跳默认 30s；若 90s 内无任何数据（含 heartbeat），服务端/代理可断开；可选启用 `CHAT_IDLE_DISCONNECT_SECONDS` 作为“LLM 长时间无输出时主动断开”的安全兜底（B 方案：生产建议先取 120s，上线后按日志 `chat_llm_first_output` 的 p99 再回调；可用 `python3 scripts/analyze_chat_llm_first_output.py logs/backend.log` 统计）；支持 last-event-id 续接。
...
- 题目定位与切片（BBox + Slice，MVP）：bbox 对象为“整题区域（题干+作答）”；允许多 bbox 列表；裁剪默认 5% padding，裁剪前 clamp 到 [0,1]；失败必须回退整页并写 warnings。切片默认 TTL=24h（`SLICE_TTL_SECONDS` 可配），Demo 可用 public bucket，生产建议 signed URL。
...
- 数据保留（产品要求）：
  - 原始图片 + 批改结果 + 识别原文的长期生命周期由上层“用户与数据管理后台”决定（本服务不实现静默清理）；
  - chat_history 与切片默认 24h TTL（会话随 `session_id`；切片由 `SLICE_TTL_SECONDS` 控制）。

## 提交前检查清单
- 字段/响应是否与 `homework_agent/API_CONTRACT.md`、`schemas.py` 一致？无多余 key。
