# Autonomous Agent Prompts (v1, 详细版)

> 本文档定义 Autonomous Grade Agent 的 Prompt 规范与工具 Schema。
> 参考：`docs/autonomous_grade_agent_design.md` / `docs/autonomous_agent_implementation.md`

---

## 1. PlannerAgent System Prompt
...
## 3. ReflectorAgent System Prompt
...
- plan_history: [{"plan": [{"step": "diagram_slice", ...}]}]
...
- plan_history: [{"plan": [{"step": "ocr_fallback", ...}]}]
...
## 4. AggregatorAgent System Prompt
...
```json
{
  "ocr_text": "9. [geometry] 10. [algebra] 11. [uncertain]",
  "results": [
    {"question_number": "9", "verdict": "incorrect", ...},
    {"question_number": "10", "verdict": "correct", ...},
    {"question_number": "11", "verdict": "uncertain", ...}
  ],
...
<wrapping_behavior>
After generating results, you MUST also populate the wrong_items array for backward compatibility:
- Filter results where verdict == "incorrect"
- Map each to wrong_items format (if required by API contract)
- uncertain verdicts should NOT appear in wrong_items but MUST appear in results
</wrapping_behavior>
...
## 5. Function Tools Schema

### diagram_slice
```json
{
  "name": "diagram_slice",
  "description": "对图片进行切片，优先 figure/question 双图",
  "parameters": {
    "type": "object",
...
{
  "name": "qindex_fetch",
  "description": "读取 session_id 对应的切片与题目索引",
  "parameters": {
    "type": "object",
...
```json
{
  ...
  "ocr_text": "...",
  "current_plan": {"plan": [...]},
  "tool_results": {"diagram_slice": {...}},
  "reflection_result": {"pass": false, "confidence": 0.72}
}
```
